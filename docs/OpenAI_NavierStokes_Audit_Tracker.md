...
Parent-directory copies are archival, and Agent 2 notes are subordinate to the
...
present in the fork `docs/` directory, the three Agent 2 notes are subordinate
supporting material, and no tracked `.olean` or `.lake` artefact is present.
This is a packaging result, not a new mathematical finding. `CTR-005` remains
open pending an explicit selected-path moment-realisation theorem.
