...
## AX-055: cutoff-gradient commutator is proved exactly

`SelectedCutoffCurlCommutator.lean` contains no admissions, custom axioms, or
unsafe declarations. It applies Mathlib's Frechet product rule to the actual
`SpatialCurl.curl` definition and isolates the term generated by the spatial
gradient of a localisation cutoff.

This is an exact local identity, not an assertion that the selected radial
integral is nonzero. The remaining work is to transport this term through the
selected finite prefix, torus average, axis and outer-support limits, and
`barMoment`.
