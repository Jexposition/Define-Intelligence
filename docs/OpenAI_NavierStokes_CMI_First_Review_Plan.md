...
|---|---:|---|
| Selected direct axis value | [x] | `SelectedRadialAxisBoundary.lean` proves component one is zero when the two radial Cartesian coordinates vanish. |
| Positive-radius branch | [x] | `SelectedRadialSectionComponent.lean` recovers the scalar coefficient for `r > 0`. |
| Discontinuity claim | [ ] | Neither branch proves a derivative mismatch or a failed smooth extension. |
| Full selected moment | [ ] | Still requires meridional curl, cutoff derivatives, torus average, outer support, and exact `barMoment` typing. |
| `Delta m` / `False` | [ ] | No selected nonzero remainder or kernel contradiction has been established. |
...
| CALC-05 | Evaluate axis and outer-support terms.               |    [~] | `SelectedRadialAxisBoundary.lean` proves the selected direct angular component-one axis value; outer-support and full mixed-field terms remain. |
...
| CALC-14 | Evaluate the atlas coefficient in the component formula. | [x] | `SelectedRadialSectionComponent.lean` recovers component one as `meanField` on the selected positive-radius section. |
...
| CALC-23 | Join the positive-radius recovery to the axis and full mixed field. | [~] | The direct angular axis branch is proved separately; meridional curl, cutoff commutator, torus average, outer support, and `barMoment` remain. |
...
| ------------------ | -------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------: |
| Lean review tree   | `lake build NavierStokesReview` has zero errors, zero `sorry`, zero custom axioms, and no `unsafe` in new review modules.  |                                                                                                                         [x] 3701 jobs under Lean `v4.34.0-rc2` |
...
| Release            | Commit and push the final selected-field audit update.                                                                     | [x] Commits `0dc5aab` and `41adfe1` are pushed to `review/cmi-first-navier-stokes-2026-09-22`; the two supplied PDFs and existing scratch file remain intentionally untracked. |
