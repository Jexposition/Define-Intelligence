...
Date: 2026-09-22
Audited commit: `f9e8bc5b38b6e212696e8a30e3e91517af887bbd`
...
| CMI source predicate mapping | PARTIALLY VERIFIED | Definitions include the classical PDE, positive viscosity, divergence, initial data, smoothness, energy, decay/periodicity, force decay, and periodic pressure. Boundary `ContDiffOn`/within-derivative equivalence and full witness-domain provenance remain human-review items. |
...
| Generic residual-rate API | ISSUE RECORDED | `JetRate` is true on the bottom filter for arbitrary data. The selected `originPast` endpoint is independently `NeBot`; a complete proof-relevant map of every derived filter is still open. |
...
includes periodic pressure in the solution structure.

## What this audit does not establish

It does not certify the analytic truth of the many imported PDE lemmas merely
because Lean accepted their interfaces. The unresolved peer-review tasks are
the correspondence between `ContDiffOn` and the CMI boundary convention, the
provenance and common-domain coverage of every inverse/correction premise, and
the proof-relevant nontriviality of all derived filters used by residual-rate
consumers. These are specific source-level review targets, not claims that a
counterexample has already been proved.
...
Current disposition: `FORMAL TRUST PASS; CMI SEMANTIC AUDIT PARTIALLY VERIFIED;
HUMAN PDE PEER REVIEW OPEN; NO NEGATIVE CMI FINDING ESTABLISHED`.
